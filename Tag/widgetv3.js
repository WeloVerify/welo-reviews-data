(() => {
  ...
  if (!document.getElementById("tagwelo-style-info-verified")) {
    const style = document.createElement("style");
    style.id = "tagwelo-style-info-verified";
    style.textContent = `
  ...
  const getEffectiveBackground = (el) => {
    while (el && el !== document.documentElement) {
      ...
      // se c'è un gradiente/immagine, consideriamo "non trasparente"
      if (bgImg && bgImg !== "none") {
  ...
  const applyTheme = () => {
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        // chiaro => widget bianco | scuro => widget nero
