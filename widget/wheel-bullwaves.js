 ...
 * Vanilla JavaScript
 *
 * Place this script after the existing Bullwaves HTML embed,
 * inside a native Webflow Form Block.
 *
 * Browser storage is not fraud prevention.
 * Claim IDs must be verified by support against Webflow submissions.
 */
...
(function () {
  ...
  const CONFIG = {
     ...
     *
     * All three prizes are always visible.
     * A weight of 0 means that the selected profile
     * cannot win that particular prize.
     *
     * Update these values when the final odds are approved.
     */
     ...
     *
     * All five challenge sizes are always visible.
     */
  ...
  const PRIZES = {
    ticket: {
      ...
      name: '1 VIP Ticket',
      value: '$1,100 VALUE',
      details: 'One ticket for A1 Combat × Bullwaves in Dubai'
    },
    ...
    bonus: {
      ...
      details: 'Tradable Bullwaves account bonus, not cash'
    },

    challenge: {
      short: ['PRIME', 'CHALLENGE'],
      name: 'Bullwaves Prime Challenge',
      value: 'UP TO $550 VALUE',
      details: 'Win a challenge account of up to $100,000'
    }
  };
  ...
  function choose(weights) {
    ...
    if (!pool.length) {
      throw new Error('No valid prize odds configured.');
    }
  ...
  function isValid(entry) {
    return (
      entry &&
      entry.version === 2 &&
      /^BW26-[0-9A-F]{8}-[0-9A-F]{8}$/.test(entry.id) &&
      CONFIG.categoryWeights[entry.type] &&
      CONFIG.categoryWeights[entry.type][entry.category] > 0 &&
      (
        entry.category !== 'challenge' ||
        CONFIG.challengeWeights[entry.type][entry.challenge] > 0
      ) &&
      ['challenge', 'ready', 'submitted'].includes(entry.stage) &&
      ['first', 'last', 'email', 'date'].every(
  ...
  function readEntry() {
    ...
    try {
      const item = document.cookie
        .split('; ')
        .find(part =>
          part.startsWith(CONFIG.storageKey + '=')
        );

      if (item) {
        cookie = JSON.parse(
          decodeURIComponent(
            item.slice(CONFIG.storageKey.length + 1)
          )
        );
      }
    } catch (_) {}
  ...
  function saveEntry(entry) {
    ...
    try {
      ...
      saved =
        localStorage.getItem(CONFIG.storageKey) === data;
    } catch (_) {}
   ...
   * No additional stylesheet is required.
   */

  function addStyles() {
    if (document.getElementById('bw-wheel-premium-styles')) {
      return;
    }
    ...
    style.textContent = `
      ...
      .bw-wheel-panel {
        position: relative;
        padding: 38px clamp(18px, 4vw, 42px) 28px;
        text-align: center;
        background:
          radial-gradient(
            ellipse at 50% 0%,
      ...
      .bw-wheel-button {
        ...
        cursor: pointer;
        box-shadow: 0 8px 35px #0036ff45;
      }
  ...
  function init() {
    ...
    if (!section || section.dataset.bwWheelReady) {
      return;
    }
    ...
    if (
      ...
      console.error(
        'Bullwaves wheel: place the existing embed inside a native Webflow Form Block.'
      );
    ...
    const profile = profileField.querySelector('select');

    Object.keys(CONFIG.categoryWeights).forEach(type => {
      profile.add(new Option(type, type));
    });
    ...
    inlineStatus.className = 'bw-wheel-inline-status';
    ...
    const heading = $('#bw-wheel-heading');

    const intro = $('.bw-wheel-intro');

    const content = $('.bw-wheel-content');

    const actions = $('.bw-wheel-actions');

    const status = $('.bw-wheel-status');

    const close = $('.bw-wheel-close');
    ...
    function addButton(label, callback, secondary) {
    ...
    function prizeValue() {
      if (entry.category === 'challenge') {
        return (
          'Retail value ' +
          money(CONFIG.challengePrices[entry.challenge]) +
          ', not cash'
        );
      }
      ...
      return 'VIP Ticket value $1,100. One ticket included';
    }
     ...
     * Zero weight categories are marked as unavailable.
     */

    function showTiles(keys, challenge, weights) {
      ...
      keys.forEach(key => {
        ...
        tile.className =
          'bw-wheel-prize' +
          (weights[key] > 0 ? '' : ' unavailable');

        const name = document.createElement('div');

        name.className = 'bw-wheel-prize-name';
        ...
        value.className = 'bw-wheel-prize-value';
        ...
        note.className = 'bw-wheel-prize-note';
        ...
        note.textContent =
          weights[key] > 0
            ...
            : 'Not eligible for your profile';

        tile.append(name, value, note);
    ...
    function drawWheel(keys, challenge) {
      const stage = document.createElement('div');

      stage.className = 'bw-wheel-stage';

      const pointer = document.createElement('div');

      pointer.className = 'bw-wheel-pointer';

      pointer.setAttribute('aria-hidden', 'true');

      const canvas = document.createElement('canvas');

      canvas.className = 'bw-wheel-disc';
      ...
      keys.forEach((key, index) => {
        ...
        words.forEach((text, line) => {
          ctx.font =
            line === words.length - 1
              ? '700 23px Inter, Arial, sans-serif'
              : '800 35px Inter, Arial, sans-serif';

          ctx.fillText(
            text,
            0,
            (line - (words.length - 1) / 2) * 45,
            245
          );
        });
      ...
      stage.append(pointer, canvas);

      content.append(stage);

      paragraph(
        'bw-wheel-eligibility',
        'All prizes are displayed. Eligibility and winning odds depend on your selected profile.'
      );
    ...
    async function spin(canvas, keys, winner) {
      setSpinning(true);
      ...
      status.textContent = 'Revealing your prize…';
    ...
    function firstWheel() {
      ...
      const weights = CONFIG.categoryWeights[type];
      ...
      const canvas = drawWheel(
        keys,
        false
      );

      addButton('Spin to Win', async () => {
        ...
        /*
         * Both results are generated and saved
         * before the first animation starts.
         */
    ...
    function secondWheel() {
      reset(
        ...
        'One more spin reveals your challenge account size. Every challenge is shown below.'
      );

      /*
       * Always show all five account sizes.
       */

      const keys = CHALLENGE_ORDER;
    ...
    function prizePanel() {
      ...
      panel.className = 'bw-wheel-win';

      const label = document.createElement('span');

      label.className = 'bw-wheel-win-label';

      label.textContent = 'YOUR WINNING PRIZE';

      const name = document.createElement('div');

      name.className = 'bw-wheel-win-title';

      name.textContent = prizeName();

      const value = document.createElement('div');

      value.className = 'bw-wheel-win-value';
      ...
      value.textContent =
        entry.category === 'challenge'
          ...
          : PRIZES[entry.category].value;

      const details = document.createElement('p');

      details.className = 'bw-wheel-win-details';

      details.textContent = prizeValue();
    ...
    function finalScreen() {
      ...
      textarea.className = 'bw-wheel-message';
